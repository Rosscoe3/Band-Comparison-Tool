...
// Parse maxY URL parameter
const maxYParam = urlParams.get("maxY");
if (maxYParam) {
  const maxYValue = parseFloat(maxYParam);
  // Validate the maxY value is within acceptable range
  if (!isNaN(maxYValue) && maxYValue >= 1 && maxYValue <= 500) {
    // Will be applied after DOM is loaded
    window.customMaxY = maxYValue;
  }
}
...
let maxY_Global = document.getElementById("maxY_Global");

// Apply custom maxY from URL parameter if present
if (window.customMaxY) {
  maxY_Global.value = window.customMaxY;
}
...
//** USE TRANSMISSION DATA FROM JAVASCRIPT FILE INSTEAD OF CSV */
// Set transmissionData directly from the imported data
transmissionData = transmissionDataPoints;
// plotCSV() and resize() will be called after charts are created
  ...
  options: {
    ...
    scales: {
      y: {
        ...
        min: 0.0,
        max: parseFloat(maxY_Global.value),
      },
      x: {
        type: "linear",
        position: "bottom",
...
// Now that charts are created, load the transmission data
plotCSV();
resize();
...
function init() {
  ...
  if (urlLayers.length > 0) {
    // Skip tutorial and collapse sidebar immediately when using URL parameters
    tutorial = false;
    tutorialIndex = 99; // Set to a high number to skip all tutorial steps
    ...
    // Hide tutorial wrapper directly instead of clicking button
    const tutorialWrap = document.getElementById("tutorial-wrap");
    if (tutorialWrap) {
      console.log("Hiding tutorial-wrap element");
      tutorialWrap.classList.remove("active");
      tutorialWrap.style.display = "none"; // Extra insurance to hide it
    } else {
      console.log("tutorial-wrap element not found");
    }
  }
  
  setTimeout(() => {
    if (urlLayers.length > 0) {
      // Load layers specified in URL
      urlLayers.forEach(layerName => {
        loadLayerByName(layerName);
      });
    } else {
      // Default behavior - load Landsat 8-9
...
// Function to parse URL parameters and extract layer names
function getLayersFromURL() {
  const urlParams = new URLSearchParams(window.location.search);
  const layers = [];
  
  // Support multiple parameter formats
  // ?layers=Landsat8-9,Sentinel-2
  // ?layer=Landsat8-9&layer=Sentinel-2
  // ?presets=Landsat8-9,Sentinel-2
  
  const layersParam = urlParams.get('layers') || urlParams.get('presets');
  if (layersParam) {
    layers.push(...layersParam.split(',').map(layer => layer.trim()));
  }
  
  // Support multiple layer parameters
  const allLayerParams = urlParams.getAll('layer');
  if (allLayerParams.length > 0) {
    layers.push(...allLayerParams.map(layer => layer.trim()));
  }
  
  return layers.filter(layer => layer.length > 0);
}

// Function to load a layer by its URL-friendly name
function loadLayerByName(layerName) {
  // Create a mapping of URL-friendly names to actual layer data
  const layerMap = {
    // Landsat family
    'landsat1-3': { name: 'Landsat 1-5 (MSS)', data: Landsat1_3_values },
    'landsat1-5': { name: 'Landsat 1-5 (MSS)', data: Landsat1_3_values },
    'landsat4-5': { name: 'Landsat 4-5 (TM)', data: Landsat4_5_values },
    'landsat7': { name: 'Landsat 7 (ETM+)', data: Landsat7_values },
    'landsat8-9': { name: 'Landsat 8-9 (OLI & TIRS)', data: Landsat8_9_values },
    'landsatnext': { name: 'Landsat Next', data: LandsatNext_values },
    
    // Sentinel family
    'sentinel-2': { name: 'Sentinel-2 (MSI)', data: Sentinel2_values },
    'sentinel2': { name: 'Sentinel-2 (MSI)', data: Sentinel2_values },
    'sentinel-3': { name: 'Sentinel-3 (OLCI)', data: Sentinel3_values },
    'sentinel3': { name: 'Sentinel-3 (OLCI)', data: Sentinel3_values },
    
    // Other satellites
    'modis': { name: 'Terra/Aqua (MODIS)', data: MODIS_values },
    'pace': { name: 'PACE (OCI)', data: PACE_values },
    'worldview-1': { name: 'Worldview-1', data: WorldView_1_values },
    'worldview-2': { name: 'Worldview-2', data: WorldView_2_values },
    'worldview-3': { name: 'Worldview-3', data: WorldView_3_values },
    'worldview-4': { name: 'Worldview-4', data: WorldView_4_values },
    'aster': { name: 'ASTER', data: ASTER_values },
    'eo1': { name: 'EO1 (ALI)', data: EO1_values },
    'desis': { name: 'DESIS', data: DESIS_values },
    'ecostress': { name: 'ECOSTRESS', data: ECOSTRESS_values },
    'emit': { name: 'EMIT', data: EMIT_values },
    'rapideye': { name: 'RapidEye', data: RapidEye_values },
    'skysat': { name: 'SkySat', data: SkySat_values }
  };
  
  // Normalize the layer name (lowercase, remove spaces and special characters)
  const normalizedName = layerName.toLowerCase()
    .replace(/\s+/g, '-')
    .replace(/[^\w-]/g, '')
    .replace(/-+/g, '-');
  
  const layer = layerMap[normalizedName];
  
  if (layer) {
    console.log(`Loading layer: ${layer.name}`);
    addPreset(layer.name, layer.data);
  } else {
...
function addPreset(title, preset)
{
  ...
  band_global_li.appendChild(band_content);
    ...
    var color_li = document.createElement('li');
    color_li.appendChild(input_container_color);
    band_content.appendChild(color_li);
    ...
    var boxHeight_li = document.createElement('li');
    boxHeight_li.appendChild(input_container_boxHeight);
    band_content.appendChild(boxHeight_li);
    ...
    var labelVisibility_li = document.createElement('li');
    labelVisibility_li.appendChild(input_container_labelVisibility);
    band_content.appendChild(labelVisibility_li);
    ...
    var labelSize_li = document.createElement('li');
    labelSize_li.appendChild(input_container_labelSize);
    band_content.appendChild(labelSize_li);
    ...
    var sublabelVisibility_li = document.createElement('li');
    sublabelVisibility_li.appendChild(input_container_sublabelVisibility);
    band_content.appendChild(sublabelVisibility_li);
    ...
    var subLabelSize_li = document.createElement('li');
    subLabelSize_li.appendChild(input_container_subLabelSize);
    band_content.appendChild(subLabelSize_li);
    ...
    var titleVisibility_li = document.createElement('li');
    titleVisibility_li.appendChild(input_container_titleVisibility);
    band_content.appendChild(titleVisibility_li);
  ...
  for(var i = 0; i < preset.length; i++)
  {
    ...
    band_item_li.appendChild(band_content);
      ...
      input_container_color.classList = "inputContainer";
      input_container_color.id = "color";
      var color_li_2 = document.createElement('li');
      color_li_2.appendChild(input_container_color);
      band_content.appendChild(color_li_2);
      ...
      if(isCustomBand)
      {
        ...
        var xMin_li_1 = document.createElement('li');
        xMin_li_1.appendChild(input_container_xMin);
        band_content.appendChild(xMin_li_1);
        ...
        var xMax_li_1 = document.createElement('li');
        xMax_li_1.appendChild(input_container_xMax);
        band_content.appendChild(xMax_li_1);
        ...
        var labelContent_li_1 = document.createElement('li');
        labelContent_li_1.appendChild(input_container_labelContent);
        band_content.appendChild(labelContent_li_1);
        ...
        var subLabelContent_li_1 = document.createElement('li');
        subLabelContent_li_1.appendChild(input_container_subLabelContent);
        band_content.appendChild(subLabelContent_li_1);
      ...
      var graphNumb_li = document.createElement('li');
      graphNumb_li.appendChild(input_container_graphNumb);
      band_content.appendChild(graphNumb_li);
      ...
      var yOffset_li = document.createElement('li');
      yOffset_li.appendChild(input_container_yOffset);
      band_content.appendChild(yOffset_li);
      ...
      if(preset[i].title == "Title")
      {
        ...
        input_container_xMin.classList = "inputContainer";
        input_container_xMin.id = "xMin";
        var xMin_li_2 = document.createElement('li');
        xMin_li_2.appendChild(input_container_xMin);
        band_content.appendChild(xMin_li_2);
...
maxY_Global.addEventListener("change", function () {
  const newMaxY = parseFloat(maxY_Global.value);
  chart.options.scales.y.max = newMaxY;
  chart2.options.scales.y.max = newMaxY;
  chart.update();
  chart2.update();
});
